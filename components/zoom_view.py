...
class ZoomView(QGraphicsView):
    ...
    def _serialize_item(self, item):
        from PyQt6.QtWidgets import QGraphicsPathItem, QGraphicsTextItem, QGraphicsItemGroup
        ...
        if isinstance(item, QGraphicsPathItem):
            ...
            for child in item.childItems():
                c_data = self._serialize_item(child)
                if c_data:
    ...
    def _deserialize_item(self, data):
        from PyQt6.QtWidgets import QGraphicsPathItem, QGraphicsTextItem, QGraphicsItemGroup
        from PyQt6.QtGui import QColor, QPen, QBrush, QTransform, QFont, QPainterPath
        ...
        if item_type == 'path':
            ...
            for c_data in data.get('children', []):
                child = self._deserialize_item(c_data)
                if child:
